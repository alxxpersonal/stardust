...
**This replaces ephemeral in-session planning.** It produces a durable plan committed in the repo, not an ephemeral in-session plan.
...
Keep the plan and its checkbox state in sync as execution proceeds: the canonical `docs/plans/` plan is the single source of truth, so any in-session view of the work always reflects what has actually been done. The plan file itself, never a plugin folder, is that source of truth.
...
- The plan instructs its executor to track its tasks with a task list, keep exactly one task in progress at a time, mark each complete immediately, and keep the task list in sync with the checkboxes.
...
## Replaces in-session planning

When you would otherwise start planning in-session, use this command instead. Follow this approval discipline:

- **Separate clarifying from approving.** Ask the user direct clarifying questions only to resolve genuine ambiguity in requirements or to choose between approaches. Do not ask "is this plan okay?" or "should I proceed?" as filler; resolve real ambiguity, otherwise proceed.
