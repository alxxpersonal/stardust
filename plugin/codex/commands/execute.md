...
**This is a durable alternative to ephemeral in-session planning.** Use it for non-trivial work. It produces a durable spec and plan committed in the repo, not an ephemeral in-session plan.
...
Approval: the explicit `/stardust:execute` invocation is itself the go-ahead to build, so go straight from step 6 into implementation. The approval discipline in "Explore before you design" below applies only to the docs-only path (the user asked to spec or plan without building) or when the user explicitly asked to review the plan before building.
...
Follow this discipline whenever you produce a spec and plan:

- **Separate clarifying from approving.** Resolve genuine ambiguity by asking the user directly to resolve requirements or choose between approaches, otherwise proceed. Do not ask "is this plan okay?" or "should I proceed?" as filler.
...
- [ ] Build proceeds on the /stardust:execute invocation with no approval gate, unless the user asked for docs only or to review first
...
- Author the spec, ADRs, and plan, then build the plan in the same turn. The `/stardust:execute` invocation is the go-ahead; do not gate the build behind an approval step unless the user asked for docs only or to review first.
