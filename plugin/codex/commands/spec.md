...
**This is a durable design workflow, not an ephemeral in-session plan.** Use it for non-trivial work. It produces a spec committed in the repo, not an in-memory plan that disappears when the session ends.
...
## Durable design discipline

When you would otherwise sketch an in-session plan, use this command instead. Follow the approval discipline:

- **Separate clarifying from approving.** Resolve genuine ambiguity by asking the user directly (requirements or a choice between approaches). Otherwise proceed. Never bury a real decision behind a vague "is this okay?".
...
- [ ] Genuine ambiguity resolved by asking the user directly, not buried behind a vague text question
