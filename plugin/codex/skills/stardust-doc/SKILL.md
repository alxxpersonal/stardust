---
name: stardust-doc
description: Use when the user asks to add one convention-correct doc to a repo - an ADR, a research note, or a runbook - or says "ADR this", "write a research note", "add a runbook", "log this decision", or wants a single quick doc under docs/. Explore with stardust, write to docs/adr, docs/research, or docs/runbooks in the docs convention, and regenerate the index. Not for a full feature spec plus plan - use the stardust-spec or stardust-execute skill for that.
---

You are running the complete convention doc workflow inline for one document. Do not invoke the unrelated Microsoft `.docx` tooling. Do not print a slash command for the user to run. Author the doc in this turn, regenerate the registry, and stop gracefully when the workspace cannot be resolved. For a full spec plus plan of non-trivial work, use the stardust-spec or stardust-execute skill instead.
...
Treat the user's request as the doc type plus topic. The type is one of `adr`, `research`, or `runbook`; the rest is the topic. If the type is invalid or unclear, state the valid set and ask which one fits. If the topic is empty, ask the user to name it and stop. Then run the workflow below verbatim from `${ROOT}`.
...
Do not skip steps. Explore before you write. Verification is mandatory. If requirements are genuinely ambiguous, resolve them with the user directly, otherwise proceed.

### 1. Pick the type and explore

- Confirm which doc type fits. If the work is a whole feature needing a spec and a plan, stop and use the stardust-spec or stardust-execute skill.
...
- Author the one doc this turn, then stop. Do not print a slash command for the user to run.
