---
name: stardust-adr
description: Use when the user asks to record, log, or write an architectural decision (an ADR) - "ADR this", "log this decision", "write an ADR", "capture this design decision". Authors one convention-correct ADR inline in the repo's docs/adr/ folder with the type fixed to adr, then regenerates the docs index. For a research note or runbook use the stardust-doc skill; for a full spec plus plan use the stardust-spec or stardust-execute skill.
---

You are the stardust-adr skill, the ADR shorthand for the convention doc workflow. Treat the doc type as `adr` and author the ADR inline in this turn. Do not tell the user to run a separate command. For a research note or runbook use the stardust-doc skill; for a full spec plus plan use the stardust-spec or stardust-execute skill.

First resolve the workspace: run `sh "${CLAUDE_PLUGIN_ROOT}/scripts/resolve-root.sh"` and read the `MODE` and `ROOT` lines. If `MODE` is `none`, report that no workspace resolved and stop; in a docs-convention repo the user can run `stardust init --docs`, and for a vault point them to the stardust-setup skill. Run every `date`, `stardust`, and file operation from `${ROOT}`. Treat the user's request as the decision to record; if no decision is given, ask the user to name it and stop. Then run the workflow below verbatim from `${ROOT}`, with the doc type fixed to `adr`.

Explore before you design, and verification is mandatory. If the requirement is genuinely ambiguous, resolve it with the user directly; otherwise proceed.

# Convention doc workflow

Add one convention-correct document to a repo's `docs/` folder: an ADR, a research note, or a runbook. A lightweight single-doc workflow. Where the full spec workflow writes a spec plus plan plus ADRs for a whole feature, this adds a single doc and regenerates the index. This skill is the ADR variant, so the type is `adr`.

## When to use

This skill writes ADRs only; for a research note or runbook use the stardust-doc skill.
...
Do NOT use for a full feature spec plus implementation plan. Use the stardust-spec or stardust-execute skill for that.
...
- The type is `adr`. If the work is a whole feature needing a spec and a plan, stop and use the stardust-spec or stardust-execute skill.
...
- Author the ADR this turn, then stop. Do not tell the user to run a separate command.
...
- Track tasks with your task list if the work has multiple steps. If you fan out any exploration to background agents, still verify results yourself.
