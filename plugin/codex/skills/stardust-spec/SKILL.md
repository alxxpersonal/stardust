---
name: stardust-spec
description: Use when the user asks to spec a feature, write a technical spec or design doc, record ADRs, or design non-trivial work before coding (triggers - "spec this", "write a spec", "design doc", "ADR this"). Explores the repo with stardust first, then writes a durable spec plus its ADRs into docs/ and regenerates the docs index. Not for trivial single-file fixes.
---

You are the stardust-spec skill, the spec authoring workflow. Author the spec and its ADRs in this turn, then regenerate the registry. This skill produces the design (the spec plus its ADRs). To write the executable plan use the stardust-plan skill, and to spec, plan, and build it all in one turn use the stardust-execute skill. Do not tell the user to run a second command.
...
Treat the user's request as the topic to spec; if no topic is given, ask the user to name the feature, topic, or decision and stop (a plain clarifying question). Then run the workflow below verbatim from `${ROOT}`, through the spec and its ADRs.

# Spec workflow

Turn a non-trivial task into a technical spec and the ADRs for its locked decisions, written into the repo's `docs/` folder in the docs convention. Explore the codebase with Stardust before proposing anything, and regenerate the docs index when done. The executable plan is written by the stardust-plan skill; the stardust-execute skill does the spec, the plan, and the build together.

This is your design gate for non-trivial work. It produces a durable spec committed in the repo, not an ephemeral in-session plan. Explore before you design, and verification is mandatory.
...
Re-read the spec with fresh eyes before considering it done, and fix inline:

- No placeholders, TBDs, or vague requirements.
- No internal contradictions.
- No requirement that could be read two ways; pick one reading and state it.
- The Work breakdown section is complete enough that the stardust-plan skill can turn it into tasks without guessing.

## Design discipline

This is your design gate. Follow this discipline:

- Separate clarifying from designing. Resolve requirements or choose between approaches with the user directly only when the requirements are genuinely ambiguous; otherwise proceed. Do not gate on a plain "is this okay?" or "should I proceed?" question.
- Do not reference "the spec" in clarifying questions before the user can see it.
...
- [ ] Work breakdown is complete enough to feed the stardust-plan skill
- [ ] Requirements resolved with the user directly only when genuinely ambiguous, not gated on a plain text approval question
...
- Track tasks with your task list as you work.
- Author the spec and its ADRs this turn, then stop. Do not tell the user to run a second command.
