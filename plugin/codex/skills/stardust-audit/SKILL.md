---
name: stardust-audit
description: Use when the user asks to audit a codebase or docs, take a pre-release or pre-handoff health pass, harden or review a subsystem or lens (security, code quality, docs coherence), or find what rotted, drifted, or got risky since last time. Harvest deterministic Stardust checks, fan out detector agents over code and policy dimensions, verify every finding adversarially, then write one verified point-in-time audit report to docs/research. Read and report only, never mutate audited code.
---

You are the workspace audit skill. It harvests the deterministic Stardust checks, fans out background agents over the dimensions no tool can reach, verifies every finding adversarially, and writes one point-in-time audit report. It proposes; it never mutates the audited code. The report is a `docs/research/` note (no `audits` collection exists, and the doc convention routes a point-in-time audit to the research type). To record a single decision the audit surfaces, use the stardust-doc skill or the stardust-adr skill; to design remediation for a non-trivial finding, use the stardust-spec skill or the stardust-execute skill. Do not tell the user to run a second command.

First resolve the workspace: run `sh "${CLAUDE_PLUGIN_ROOT}/scripts/resolve-root.sh"` and read the `MODE` and `ROOT` lines. If `MODE` is `none`, report that no workspace resolved and stop; in a docs-convention repo the user can run `stardust init --docs`, and for a vault point them to the stardust-setup skill. Run every `date`, `stardust`, and file operation from `${ROOT}`. Treat the user's request as the audit scope (a path, a subsystem, or a lens such as "security" or "docs"); if no scope is given, audit the whole workspace at default depth. An empty scope is not a blocker, it is the full-repo default. Then run the workflow below verbatim from `${ROOT}`.

# Audit workflow

Take a point-in-time read of the workspace, surface the real problems with evidence, and leave a durable report. The deterministic dimensions (docs hygiene, freshness, links, routing) are already a Stardust tool: invoke and summarize them, do not reimplement them. The dimensions that need code semantics (correctness, security, quality, policy) get an agent fan-out, because Stardust indexes markdown, not code, by design. Every candidate finding is then verified adversarially before it reaches the report.

**This is a read-and-report skill.** It writes one research note and regenerates the index. It MUST NOT edit, fix, or refactor the audited code. Findings are a proposal the user acts on, in the librarian discipline: notify, question, or review, never a silent mutation.

Explore before you design, and verification is mandatory. If the scope is genuinely ambiguous, resolve it with the user directly; otherwise proceed at the default depth.
...
- Designing or building a fix. The audit names problems; the stardust-spec skill and the stardust-execute skill design and build the remediation.
- A single known bug with a clear fix. Fix it directly.
- Recording one decision or one research finding. Use the stardust-doc skill.
...
- Stardust is optional but recommended. With it, the deterministic dimensions are a CLI call and the index is automated. Without it, fall back to grep and find for the cheap checks, run the agent dimensions anyway, and skip the registry step.
- The deterministic dimensions assume Stardust owns them. Code semantics, secrets, dependency health, and policy adherence are out of Stardust's lane and are why the agent fan-out exists.
...
Two families. Family A is deterministic: Stardust (or a grep fallback) already settles it, so the skill invokes the tool and triages its output. Family B needs code semantics or policy reading, so the skill fans out one detector agent per dimension. Scope and the user's ask decide which fire.
...
### Family B: code and policy (fan out one detector agent per dimension)
...
- Resolve scope from the user's request: a path, a subsystem, or a lens narrows Family B to the dimensions and files in scope; an empty scope means the whole workspace at default depth. Detect the toolchain (Go, Node, mixed) so the dependency, complexity, and coverage priors in Family B can run.
...
- These results are already truth and need no agent. The mechanical kinds (`forbidden-dash`, `bad-doc-name`, `bad-doc-field`, `broken-doc-ref` existence, `bad-target`) ship straight to the report as confirmed. The judgment kinds (`drift`, `stale-governed-doc`) are review-prompts by design (ADR 0018), so they enter Phase 3 verification as candidates, not facts.
...
### 3. Detect: fan out one agent per Family B dimension

- For each in-scope Family B dimension, dispatch one detector background agent on a capable model. Hand each agent a tight scope: the relevant files and globs for its dimension (found via grep, glob, and `stardust bundle`), and for the dimensions with a deterministic prior (B8, B9, B10, B11, B13) the tool output to triage rather than trust verbatim.
...
- For every candidate (Family B detector output, plus the Family A judgment kinds `drift` and `stale-governed-doc`), dispatch a separate verifier background agent, a different instance from the detector so it is not anchored on the detector's reasoning. The verifier receives only the claim plus the file pointers and re-derives from source.
...
- Apply the baseline ratchet: diff against the prior audit note or `.stardust/baseline.json` and mark each finding new or pre-existing. New crit and high findings are the headline; pre-existing ones are standing backlog. This keeps the skill adoptable on an already-dirty repo.
...
- **Recommendation**: prioritized remediation. Crit and high map to fix-now and SHOULD point at the stardust-spec skill for the remediation design; med and low map to a follow-up or the deferred list. Each item references a finding ID.
...
- **Propose, do not mutate.** The skill writes a report and surfaces crit and high findings; it does not touch the audited code. Remediation is a separate, user-initiated step through the stardust-spec skill or the stardust-execute skill.
...
- [ ] Family B detection fanned out one agent per in-scope dimension, detectors read code and did not mutate
...
- Write the report this turn, then stop. Do not tell the user to run a second command.
