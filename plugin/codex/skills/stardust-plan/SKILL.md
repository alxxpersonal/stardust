---
name: stardust-plan
description: Use when the user asks to plan work, write an executable plan, turn a spec into an implementation plan, replace plan mode, or list active plans - author a durable, tickable plan into docs/plans and regenerate the docs index.
---

# Stardust executable plan skill

This is the stardust-plan skill, the executable-plan surface. With no topic, list active plans from the resolved workspace. With a topic, author the executable plan in this turn, then regenerate the registry. This skill produces the plan; the spec and its ADRs come from the stardust-spec skill, and the stardust-execute skill does the spec, the plan, and the build together. Do not print any slash command for the user to run.

## Workspace resolution (do this first)

Run `sh "${CLAUDE_PLUGIN_ROOT}/scripts/resolve-root.sh"` and read the `MODE` and `ROOT` lines. If `MODE` is `none`, report that no workspace resolved and stop; in a docs-convention repo the user can run `stardust init --docs`, and for a vault point them to the stardust-setup skill. Run every `date`, `stardust`, and file operation from `${ROOT}`.

## Argument handling

Treat the user's request as the topic. If no topic is given, list active plans instead of writing:

- Read `${ROOT}/docs/INDEX.md` if present, extract the rows from the `## Plans` table, drop the settled statuses (`Done`, `Abandoned`, `Implemented`, `Superseded`, `Archived`), and print the title, status, and path for each remaining plan.
- If `docs/INDEX.md` is missing or has no `## Plans` table, fall back to globbing `${ROOT}/docs/plans/*.md` and reading each file's YAML `title` and `status`, then apply the same settled-status filter.
- If none are active, say so and ask for a topic. Stop without writing files.

If a topic is present, run the workflow below verbatim from `${ROOT}`.
...
This replaces ad-hoc plan mode. It produces a durable plan committed in the repo, not an ephemeral in-session plan.
...
- Find the spec this plan implements: `stardust query "<topic>"` surfaces a matching spec, plan, or ADR. If a spec exists, read it and base the plan on its Approach and Work breakdown. If no spec exists, plan directly from the topic but keep the plan self-contained; and when that spec-less work is design-heavy or depends on external knowledge, recommend the stardust-spec skill or the stardust-execute skill first for the full research and design rigor, then plan from that spec.
...
- The plan instructs its executor to track tasks with its own task list, keep exactly one task in progress at a time, mark each complete immediately, and keep the task list in sync with the checkboxes.
...
Re-read the plan with fresh eyes before finishing, and fix inline:
...
When you would otherwise draft an ephemeral plan, use this skill instead. Follow this discipline:

- Resolve requirements with the user directly only when they are genuinely ambiguous, or when choosing between real approaches. Otherwise proceed. Do not ask "is this plan okay?" or "should I proceed?" as busywork.
- Verification is mandatory. Every plan ends with how to prove the change works end to end (build, tests, run the app, tools). Be adversarial: the goal is to break it, not to confirm the happy path.
- Explore before you design. No proposal before the exploration in step 1.
- Do not make large assumptions about intent. Tie loose ends with the user before writing the plan, not during implementation.
...
- With no topic, list active plans and stop. With a topic, author the executable plan this turn, then stop. Do not print any slash command for the user to run.
