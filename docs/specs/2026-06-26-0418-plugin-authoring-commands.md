---
title: Plugin authoring commands for the spec-forge and doc-forge write path
status: Draft
version: 1
date: 2026-06-26
related:
...
# Plugin authoring commands for the spec-forge and doc-forge write path

Four new slash commands (`/stardust:spec`, `/stardust:plan`, `/stardust:doc`, `/stardust:adr`) close the authoring loop by routing the user into the canonical spec-forge and doc-forge skills to write specs, plans, ADRs, and single docs into the repo docs convention, while the plugin keeps reading that same state at session start.
...
Canonical skill homes. The spec-forge, doc-forge, and stardust skills are canonical in forge-private and symlinked into the user skills dir:

- `~/.claude/skills/spec-forge -> /Users/alxx/Code/Self/forge-private/skills/spec-forge`
- `~/.claude/skills/doc-forge -> /Users/alxx/Code/Self/forge-private/skills/doc-forge`
- `~/.claude/skills/stardust -> /Users/alxx/Code/Self/forge-private/skills/stardust`

There is exactly one copy of each skill on disk. Any second copy is drift.

spec-forge already owns the discipline. Per its SKILL.md it explores with stardust first, writes `docs/specs/<ts>-<slug>.md`, spins ADRs at `docs/adr/<NNNN>-<slug>.md`, writes the executable plan to the single canonical location `docs/plans/<ts>-<slug>.md`, runs `stardust registry`, and self-reviews. It explicitly replaces native plan mode and requires the native `/plan` to track the canonical `docs/plans/` plan and its checkbox state, surfaced via ExitPlanMode.

doc-forge owns the single-doc path. It writes one ADR, research note, or runbook to the right folder with convention-correct frontmatter and collapsible sections, then regenerates the index. It defers full feature work to spec-forge.
...
Distribution asymmetry. The plugin is publishable. The forge skills are private. A public consumer of the plugin may not have spec-forge or doc-forge installed. The commands must degrade to a documented pointer rather than error when a named skill is absent.
</details>
...
1. The plugin exposes four authoring commands that route the user into the canonical write workflows: `/stardust:spec` and `/stardust:plan` into spec-forge, `/stardust:doc` and `/stardust:adr` into doc-forge.
...
3. The produced plan is linked to Claude native `/plan`: the canonical `docs/plans/` file is the source of truth and the native in-session plan tracks it, inherited from spec-forge's existing sync.
...
- No move of spec-forge, doc-forge, or stardust into the plugin tree (the canonical-home restructure). That is option (b) in ADR 0020 and is explicitly rejected here.
...
|------|---------|-----------|------|
| `spec.md` | `/stardust:spec [topic]` | spec-forge | full spec, ADRs, plan |
| `plan.md` | `/stardust:plan [topic]` | spec-forge (or list) | list active plans, or start a spec+plan |
| `doc.md` | `/stardust:doc [adr\|research\|runbook] [topic]` | doc-forge | one convention-correct doc |
| `adr.md` | `/stardust:adr [decision]` | doc-forge | ADR shorthand |
...
Delegation, not invocation. After preconditions, the command delegates to the named canonical skill. The normative mechanism is a terminal handoff: the command's final output is the exact skill invocation to run next, for example `/spec-forge "<topic>"` or `/doc-forge adr "<decision>"`. If a future harness exposes the Skill tool to commands, a command MAY add `Skill` to its `allowed-tools` and invoke the canonical skill in the same turn; the handoff remains the fallback. Either way the canonical skill runs with its full, unmodified discipline. The command MUST NOT reproduce the skill's writing steps inline (ADR 0021).
...
4. Delegate to spec-forge with the topic. The command states that spec-forge explores with stardust, writes `docs/specs/`, `docs/adr/`, and `docs/plans/`, runs `stardust registry`, and keeps native `/plan` in sync.

`/stardust:plan [topic]`:
1. Resolve workspace; stop on `none`.
2. If `$ARGUMENTS` is empty: list active plans from the `## Plans` table in `docs/INDEX.md`, dropping settled statuses (`Done`, `Abandoned`), most recent first, each with its path. If none are active, say so and point to `/stardust:spec "<topic>"` to start one. The command surfaces paths; re-hydrating a plan into the native `/plan` view is done by a follow-on spec-forge run or the agent reading the file, not by the command turn.
3. If `$ARGUMENTS` is provided: delegate to spec-forge with the topic. The plan is the deliverable; spec-forge writes it to the canonical `docs/plans/` and syncs native `/plan` (ADR 0022).
...
4. For `adr`, compute and show the next free zero-padded four-digit number by listing `docs/adr/` and incrementing the max, as a convenience hint only. doc-forge remains the authority that assigns the final number.
5. Delegate to doc-forge with `<type> <topic>`.

`/stardust:adr [decision]`:
1. Resolve workspace; stop on `none`.
2. If `$ARGUMENTS` is empty, ask the user to name the decision.
3. Compute the next ADR number as a hint.
4. Delegate to doc-forge with `adr <decision>`. This is exactly `/stardust:doc adr <decision>` with a shorter path for inline decision capture.

Native `/plan` linkage. The link is inherited, not re-built. spec-forge already writes the canonical plan to `docs/plans/` and, inside native plan mode, mirrors it through ExitPlanMode so the in-session checkbox state tracks the file. `/stardust:spec` and `/stardust:plan` route into spec-forge, so they inherit that sync for free. `/stardust:plan` with no topic is the read affordance over `docs/plans/`, the symmetric write-side counterpart to the `<active-plans>` block the SessionStart hook already injects. ADR 0022 records that `docs/plans/` is canonical and native `/plan` is its mirror.

No drift. The commands name the canonical skills that live in forge-private and are symlinked into the user skills dir (ADR 0020, option a). The plugin ships no copy of any forge skill. There is one writing discipline, in the skill, read by both the command-driven write path and the hook-driven read path.

Graceful degradation. When `MODE` is `none`, the command stops with the setup pointer. When the stardust binary is absent, the surfacing steps that shell out (registry reads) degrade to a note, matching `status.md`. When a named forge skill is not installed (a public plugin consumer without forge-private), the handoff names the skill and additionally points at the docs convention folders so the user can author by hand. The commands never error loudly and never nag.
</details>
...
Commands write docs directly (thick commands). Each command embeds the doc convention (frontmatter, sections, collapsibles, ADR numbering) and writes with the Write tool. Rejected: this forks the convention into four command bodies that drift from spec-forge and doc-forge the moment either skill changes. It is the precise failure the canonical-skill setup prevents.

Make the plugin the canonical home for the trio (option b). Move spec-forge, stardust, and doc-forge into `plugin/claude/skills/` and have forge-private symlink to the plugin. Rejected as the default: it couples the private skill home to the plugin's release cadence, risks shipping private skills inside a publishable plugin, and is a large restructure for no gain over referencing. Reconsider only if a deliberate canonical-home consolidation is chosen (ADR 0020).
...
Auto-fire a skill from the SessionStart hook. Rejected: violates the no-nag posture and the skill discipline (spec-forge must not auto-fire on trivial work). The write path is user-initiated by design.
</details>
...
- Forge skills absent for a public plugin user. The full workflow needs spec-forge and doc-forge, which are private. Mitigation: the handoff degrades to naming the docs convention folders; the read side (SessionStart injection) keeps working regardless.
...
- Native-plan sync depends entirely on spec-forge. If spec-forge changes how it syncs ExitPlanMode, the commands inherit the change silently. Acceptable: that is the point of single sourcing, and it is recorded in ADR 0022.
- ADR number races. Two authors writing ADRs concurrently can collide on the next number. This spec mitigates by taking the distinct 0020-0022 range, leaving 0014-0019 for the sibling coherence spec; doc-forge remains the final number authority at write time.
</details>
...
3. Repo happy path. In this repo (repo mode), `/stardust:spec "demo topic"` surfaces recent specs and ends by naming the spec-forge invocation. `/stardust:doc adr "demo decision"` validates the type, prints the next ADR number, and names the doc-forge invocation. Neither command writes a file itself (confirm `git status` is unchanged by the command turn).
...
6. Native-plan linkage. Running the named spec-forge workflow inside native plan mode produces a `docs/plans/<ts>-<slug>.md` whose checkbox state is mirrored in the in-session plan view via ExitPlanMode.
...
- Any change to spec-forge, doc-forge, or stardust skill internals.
...
1. `plugin/claude/commands/spec.md`: route to spec-forge, surface recent specs.
2. `plugin/claude/commands/plan.md`: list active plans or route to spec-forge.
3. `plugin/claude/commands/doc.md`: parse and validate type, route to doc-forge, hint next ADR number.
4. `plugin/claude/commands/adr.md`: ADR shorthand over doc-forge.
...
- spec-forge SKILL.md (sections: Process, Replaces native plan mode, Voice and formatting)
- doc-forge SKILL.md (Doc types, Process)
