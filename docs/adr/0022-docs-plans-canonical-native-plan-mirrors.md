...
`docs/plans/<ts>-<slug>.md` is canonical. Native `/plan` is a synced view of it, owned by the spec workflow, which keeps the in-session checkbox state tracking the file via ExitPlanMode. `/stardust:plan` is the read affordance: with no topic it lists active plans from the registry with their paths; with a topic it routes into the spec workflow to start a new spec and plan. `/stardust:plan` MUST NOT write a plan itself, and no plan is stored anywhere outside `docs/plans/` (no `docs/superpowers/` or other mirror folder).
...
- Native-plan checkbox state stays truthful because the spec workflow syncs it from the canonical file as execution proceeds.
- The read side (hook injection) and the write side (`/stardust:plan` plus the spec workflow) are symmetric over the same registry.
- The linkage depends on the spec workflow's sync; a change there propagates to every consumer, which is the intended single-sourcing.
...
- Mirroring the plan into a second folder for the native view: forbidden by the spec workflow's single-canonical-location rule; rejected.

## References

- docs/specs/2026-06-26-0418-plugin-authoring-commands.md
- the spec workflow SKILL.md (Replaces native plan mode)
