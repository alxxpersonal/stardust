...
- **The fallback (the conservative spine).** Compute a confident subset = matched mounts union metadata-less mounts. When routing engaged but pruned nothing, fall back to ALL with mode `fallback`: the subset is empty, the subset equals the full set, or embeddings were wanted but the query embed failed while metadata is present. When routing cannot engage at all - one or zero mounts, or no mount carries any metadata - the mode is `all` (routing never ran), which keeps those workspaces byte-identical and renders no routing line. Route only when the plan is a strict, non-empty subset of the mount set.
...
- Metadata-less workspaces stay byte-identical too: with no descriptions on any mount, there is nothing to route on, so the mode is `all`, no routing line renders, and every mount is searched. This makes the design honor SPEC 12.1's "fan-out beats a router at this scale" automatically, and routing only begins pruning once mounts carry descriptions and a real strict subset emerges.
