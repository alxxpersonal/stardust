...
- **MCP shipped with the mounts/superpower layer**, packaged as a Claude Code plugin (`stardust serve --mcp`). MCP's real payoff is the connector ecosystem (the mounts), which is why MCP arrived WITH mounts, not in v1. As shipped: sharp WHEN/WHEN-NOT tool descriptions and a git-committed `.mcp.json` (NOT resources - they don't auto-load).
...
Agent infrastructure is synchronized through `.stardust/sync.toml`. `stardust sync` discovers skills, agents, and rules, honors optional frontmatter `targets: [claude, codex, gemini]`, plans repo/global target changes, supports `--dry-run`, `--check`, and `--repair`, and refuses unmanaged conflicts. `stardust sync init --profile alxx` scaffolds the local migration profile; `stardust sync report` lists import-only and loose assets to adopt. Rules adapter sync ships: a single canonical `.stardust/rules.md` is rendered per tool through a format-aware adapter map and composed into `CLAUDE.md` (claude), `AGENTS.md` (codex), and `GEMINI.md` (gemini) as a sentinel-delimited managed block, never a blind symlink. Compose owns only its block, so user content outside it survives every sync; a missing block counts as missing and a stale block as drift for `--check`, and a plain `sync` self-heals compose drift without `--repair`.
